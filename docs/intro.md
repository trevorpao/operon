...
這是 geneEH 的 boilerplate 範本。

一個整合了 **Vite**、**Bulma (綠色系主題)**、**Handlebars.js (符合 CSP 安全規範)** 以及 **geneEH (行為驅動邏輯)** 的完整初始化開發範例。
...
> **注意**：請將 `gene.min.js` 放置於 `src/libs/` 目錄下以供後續引入。
...
gee.hook('sendAction', function (me) {
  ...
  return false; // 若不想讓瀏覽器進行實際提交可 return false
});
...
document.addEventListener('DOMContentLoaded', () => {
  gee.init(); // 掃描 .gee 元素並執行對應基因（data-gene）
});
```

HTML 參考（改為 submit 綁定）：
```html
